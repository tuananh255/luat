...
interface EditHumanResourcesProps {
  onClose?: () => void;
  isOpen?: boolean;
}

const EditHumanResources: React.FC<EditHumanResourcesProps> = ({
  onClose,
  isOpen,
}) => {
      ...
      >
        <div className="self-stretch h-max flex-col justify-start items-start gap-4 flex">
          <div className="flex-col justify-center items-start gap-[17px] flex w-full">
            <div>
              <strong className="text-[18px]">Thêm thành viên</strong>
            </div>
            ...
            <div className="w-full">
              ...
              <Input placeholder="Kinh nghiệm làm việc" />
            </div>
            <div className="w-full">
              <label>Thành tích nỗi bật</label>
              <Input placeholder="Thành tích nỗi bật" />
            </div>
...
export default EditHumanResources;
